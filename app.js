...
const StorageEngine = {
    ...
    async clearAndSave(storeName, data) {
        if (!this.db) await this.init();
        if (!this.db || !this.db.objectStoreNames.contains(storeName)) return;
...
const db = {
    ...
    async load() {
        ...
        // الـ booking groups الثابتة أُزيلت — المجموعات تُدار يدوياً فقط
...
function renderGroupSelection(gradeId) {
    ...
    let html = `
        <div class="grade-card-modern fade-in" onclick="openAddGroupModal()" style="--accent-color: var(--primary); background: rgba(255,255,255,0.05); border: 2px dashed rgba(255,255,255,0.2);">
...
async function deleteGrade(id) {
    ...
    try {
        ...
        await StorageEngine.clearAndSave('groups', db.groups);
...
// يُستخدم لتتبّع إن كنا في وضع "تعديل" مجموعة موجودة أو "إضافة" مجموعة جديدة
let editingGroupId = null;

function openAddGroupModal() {
    editingGroupId = null;
    document.getElementById('group-modal-title').innerText = 'بيانات المجموعة الجديدة';
    document.getElementById('group-name').value = '';
    document.getElementById('group-time').value = '';
    document.getElementById('group-save-btn').innerText = 'حفظ المجموعة';
    toggleModal('group-modal', true);
}

function editGroup(id) {
    const group = db.groups.find(g => String(g.id) === String(id));
    if (!group) return showNotification('تعذّر العثور على المجموعة', 'error');

    editingGroupId = id;
    document.getElementById('group-modal-title').innerText = 'تعديل بيانات المجموعة';
    document.getElementById('group-name').value = group.name;
    document.getElementById('group-time').value = group.time;
    document.getElementById('group-save-btn').innerText = 'حفظ التعديلات';
    toggleModal('group-modal', true);
}
...
function handleAddGroup() {
    ...
    if (editingGroupId !== null) {
        // ─── وضع التعديل ───────────────────────────────────────
        const group = db.groups.find(g => String(g.id) === String(editingGroupId));
        if (!group) {
            showNotification('تعذّر العثور على المجموعة لتعديلها', 'error');
            editingGroupId = null;
            return;
        }
        group.name = name;
        ...
        editingGroupId = null;
        finishGroupModalFlow('✅ تم تعديل المجموعة بنجاح');
        return;
    }
    ...
    finishGroupModalFlow('✅ تم إضافة المجموعة بنجاح');
}

function finishGroupModalFlow(message) {
    // UI Updates
    renderGroups();
        ...
        <tr>
            ...
            <td>
                <div style="display:flex; gap:10px;">
                    ...
                    <button class="btn" style="color:var(--primary)" onclick="editGroup(${g.id})" title="تعديل المجموعة">
                        <i class="fas fa-pen"></i>
                    </button>
                    <button class="btn" style="color:var(--danger)" onclick="deleteGroup(${g.id})" title="حذف المجموعة">
                        <i class="fas fa-trash"></i>
                    </button>
...
async function renderPortalGroups(gradeId) {
    ...
    html += `
        <div class="grade-card-modern fade-in" onclick="openAddGroupModal()" style="--accent-color: var(--secondary); border: 2px dashed rgba(0,0,0,0.1); background: #f8fafc; color: var(--text-main); height: 260px; width: 220px; cursor: pointer;">
...
function renderDailyTreasuryArchives() {
    ...
    list.innerHTML = archives.map(a => {
        const safeId = Math.round(Number(a.id));
        return `
        <div class="card fade-in" style="padding: 1.5rem; border-right: 5px solid var(--accent); cursor: pointer;" onclick="viewDailyArchive(${safeId})">
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <div>
                    ...
                    <div style="color:var(--text-muted); font-size:0.9rem; margin-top:5px;">
                        إجمالي التحصيل: <b style="color:var(--accent)">${(a.totalSub||0) + (a.totalMisc||0)} ج.م</b>
                        ${a.totalExp ? ` | مصروفات: <b style="color:var(--danger)">-${a.totalExp} ج.م</b>` : ''}
                        | صافي: <b style="color:var(--primary)">${a.total} ج.م</b>
                    </div>
                </div>
                <div style="text-align:right;">
                    <div style="font-size:0.8rem; color:var(--text-muted)">اشتراكات: ${a.totalSub||0} | ملازم: ${a.totalMisc||0}</div>
                    <button class="btn" style="margin-top:10px; background:var(--primary); color:#fff; padding:6px 16px; border-radius:8px;">
...
function viewDailyArchive(archiveId) {
    const archive = (db.dailyTreasuryArchives || []).find(a =>
        Math.round(Number(a.id)) === Math.round(Number(archiveId))
    );
    if (!archive) {
...
function printMonthlyReceipt(paymentId, size = 'thermal') {
    ...
    if (size === 'normal') {
        ...
        <body>
            <div class="center">
                <h3 style="margin:5px 0;">نظام إدارة الدروس</h3>
                <div>${cycleTitle}</div>
            </div>
            <hr>
            <table>
...
function sendWhatsApp(studentId, target) {
    ...
    let msg = `*🏷️ تقرير المتابعة - نظام إدارة الدروس*\nالطالب: ${s.name}\nحضر: ${atts.length} حصة\nنتمنى لكم دوام التفوق.`;
...
async function exportData() {
    try {
        ...
        const fileContent = `/** 
 * نظام إدارة الدروس Permanent Data Backup - (data.js)
...
const APP_THEME_KEY = 'edu_system_theme';
const APP_THEMES = [
...
function renderProgramSettings() {
    ...
    if (printWidth) printWidth.value = localStorage.getItem('edu_system_print_width') || '80mm';
...
function saveProgramSettings() {
    ...
    localStorage.setItem('edu_system_print_width', printWidth);
...
async function importPlatformCourseCodes() {
    showNotification('هذه الميزة غير متاحة في النسخة Offline', 'info');
}
...
const exposures = {
    ...
    renderDailyTreasury, renderDailyTreasuryArchives, manualResetDailyTreasury,
    viewDailyArchive, printDtArchiveDetail,
    ...
    // Data & Sync
    exportData, importData, importFromFolder, showCycleArchive, viewArchivedCycle,
...
function generatePrintableIDCards(students, mode = 'normal') {
    ...
    if (isThermal) {
        students.forEach(s => {
            ...
            html += '<div class="page">' +
                '<div class="card">' +
                '<div class="header-text">نظام إدارة الدروس</div>' +
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (splash) {
        ...
        // ── DEMO MODE: Skip password, auto-enter after 2s ──
        if (window.DEMO_MODE) {
            // Splash already auto-dismisses via its own JS after 4s
